...
Die Fotos aus dem Reise-Tagebuch liegen als Dateien unter `~/campmesser/uploads/trips/`, die Fotos eigener Rezepte unter `~/campmesser/uploads/recipes/`, die Fotos der Zeltplatz-Dossiers unter `~/campmesser/uploads/spots/`, die Fotos der Inventar-Gegenstände unter `~/campmesser/uploads/inventory/`, die Belege (Kaufquittungen) der Inventar-Gegenstände unter `~/campmesser/uploads/receipts/` und die Fotos der Natur-Beobachtungen unter `~/campmesser/uploads/sightings/` (alles neben `dist/`), nur die Metadaten stehen in der Datenbank. Das Verzeichnis ist bewusst nicht versioniert (`.gitignore`); das Deployment per `git pull` und das Deploy-Skript lassen es unangetastet – falls du je auf ein Sync-Verfahren wie `rsync --delete` umstellst, musst du `uploads/` zwingend ausnehmen (`--exclude 'uploads/'`), sonst gehen die Fotos verloren. **Backup-Hinweis:** Der `mysqldump` sichert die Fotos nicht mit – sichere `uploads/` zusätzlich, z. B. gelegentlich per `tar czf ~/backups/campmesser/uploads-$(date +%Y%m%d).tar.gz -C ~/campmesser uploads` oder per SFTP-Download auf einen anderen Rechner. Das gilt besonders für `uploads/receipts/`: Kaufbelege lassen sich – anders als ein Gegenstands-Foto – nach einem Datenverlust nicht einfach neu aufnehmen, sichere dieses Verzeichnis also mit derselben Sorgfalt wie den Datenbank-Dump.

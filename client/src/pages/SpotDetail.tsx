...
async function fetchDossierWeather(lat: number, lon: number): Promise<DossierWeather> {
  ...
  const hourly: HourlyWeather[] = (json.hourly.time as string[]).map((time: string, i: number) => ({
    time,
...
export default function SpotDetailPage() {
  ...
  const tripStats = useMemo(
    () =>
      computeTripStats(
        spotTrips.map(t => ({ startDate: t.startDate, endDate: t.endDate, placeName: "x" })),
      ),
    [spotTrips],
  );
  ...
  if (!isAuthenticated) {
    return (
      <div className="container py-6">
        <PageHeader title="Zeltplatz" backHref="/zeltplaetze" backLabel="Zeltplätze" />
  ...
  return (
    <div className="container max-w-3xl py-6">
      <PageHeader title={spot.name} backHref="/zeltplaetze" backLabel="Zeltplätze" />
      <p className="mb-1 flex items-center gap-1.5 text-sm text-muted-foreground">
        <MapPin className="h-3.5 w-3.5" aria-hidden="true" />
        {spot.latitude.toFixed(4)}°, {spot.longitude.toFixed(4)}°
      </p>
      {spot.note && <p className="mb-4 text-sm text-muted-foreground">{spot.note}</p>}
        ...
        <CardContent>
          <div className="grid grid-cols-3 gap-3 text-center">
            <div className="rounded-lg bg-accent/50 py-2.5">
              <p className="font-mono text-lg font-bold">{fmtTime(sun?.sunrise ?? null)}</p>
              <p className="text-xs text-muted-foreground">Aufgang</p>
            </div>
            <div className="rounded-lg bg-accent/50 py-2.5">
              <p className="font-mono text-lg font-bold">{fmtTime(sun?.solarNoon ?? null)}</p>
              <p className="text-xs text-muted-foreground">Höchststand</p>
            </div>
            <div className="rounded-lg bg-accent/50 py-2.5">
              <p className="font-mono text-lg font-bold">{fmtTime(sun?.sunset ?? null)}</p>
      ...
      <Card className="mb-4">
        ...
        <CardContent>
          ...
          {!weather && !weatherFailed && <Skeleton className="h-24 w-full rounded-lg" />}
          {weather && (
            <>
              {weather.alerts.length > 0 ? (
                ...
                >
                  ...
                  {weather.alerts.length > 1 && ` (+${weather.alerts.length - 1} weitere)`}
                </p>
              ) : (
              ...
              <div className="divide-y divide-border/60">
                {weather.daily.map((d, i) => (
                  <div key={d.date} className="flex items-center gap-3 py-2 text-sm">
        ...
        <CardContent>
          {obstacles.length > 0 ? (
            <p className="text-sm text-muted-foreground">
              {obstacles.length}{" "}
              {obstacles.length === 1 ? "Hindernis erfasst" : "Hindernisse erfasst"} – Schattenzeiten
              und Panel-Ausrichtung berücksichtigen dieses Profil automatisch.
            </p>
          ) : (
            <p className="text-sm text-muted-foreground">
              Für diesen Platz ist noch kein Hindernis-Profil erfasst. Trage Bäume, Berge oder
              Gebäude im Sonnen-Kompass ein, um Schattenzeiten zu sehen.
            </p>
          )}
          <Link
            href={`/sonne?lat=${spot.latitude}&lon=${spot.longitude}&name=${encodeURIComponent(spot.name)}&spot=${spot.id}`}
          ...
          >
            <Compass className="h-4 w-4" aria-hidden="true" />
            Profil im Sonnen-Kompass {obstacles.length > 0 ? "bearbeiten" : "anlegen"}
          </Link>
        </CardContent>
      </Card>
          ...
          {spotTrips.length > 0 ? (
            <>
              <div className="mb-3 grid grid-cols-2 gap-3 text-center">
                ...
                <div className="rounded-lg bg-accent/50 py-2.5">
                  <p className="font-serif text-xl font-bold">{spotTrips.length}</p>
                  <p className="text-xs text-muted-foreground">
              ...
              <ul className="space-y-1.5">
                {spotTrips.slice(0, 3).map(trip => (
                  ...
                  >
                    ...
                    {new Date(`${trip.startDate}T00:00:00`).toLocaleDateString("de-CH", {
                      day: "numeric",
                      month: "short",
                      year: "numeric",
                    })}{" "}
                    · {tripNights(trip.startDate, trip.endDate)}{" "}
                    {tripNights(trip.startDate, trip.endDate) === 1 ? "Nacht" : "Nächte"}

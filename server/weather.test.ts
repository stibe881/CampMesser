...
describe("detectAlerts", () => {
  ...
  it("erkennt Sturmböen als Gefahr", () => {
    const hours = Array.from({ length: 48 }, (_, i) => hour({ windGustsKmh: i === 5 ? 95 : 20 }, i));
    const alerts = detectAlerts(hours);
    expect(alerts.some(a => a.id === "sturm" && a.severity === "gefahr")).toBe(true);
  });

  it("erkennt Gewitter über den WMO-Code", () => {
    const hours = Array.from({ length: 48 }, (_, i) => hour({ weatherCode: i === 10 ? 95 : 1 }, i));
    const alerts = detectAlerts(hours);
  ...
  it("erkennt Starkregen und sortiert Gefahr vor Info", () => {
    const hours = Array.from({ length: 48 }, (_, i) =>
      hour({ precipitationMm: i === 3 ? 20 : 0, temperatureC: i === 30 ? -2 : 15 }, i),
    );
    const alerts = detectAlerts(hours);
...
describe("calcWaterNeeds mit Hunden und Körperpflege", () => {
  it("rechnet Hunde-Wasserbedarf ein", () => {
    const base = calcWaterNeeds({
      adults: 2, children: 0, dogs: 0, days: 2, maxTempC: 20, activity: "ruhig", includeCookingHygiene: false,
    });
    const withDog = calcWaterNeeds({
      adults: 2, children: 0, dogs: 1, days: 2, maxTempC: 20, activity: "ruhig", includeCookingHygiene: false,
    });
  ...
  it("rechnet komfortable Körperpflege mit 4 l pro Person und Tag", () => {
    const result = calcWaterNeeds({
      adults: 2, children: 1, days: 3, maxTempC: 18, activity: "ruhig",
      includeCookingHygiene: false, includeComfortHygiene: true,
    });
